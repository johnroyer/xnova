...
/**
  ...
  */
    function info_playerList()
    {
        ...
        if (!$sRes = $this->_fastcall("PL")) {
            return false;
        }
        if ($sRes == CYTS_INVALID_PARAM) {
            return false;
        }
        $sList = explode("\r\n", $sRes);
        array_shift($sList);
...
/**
  ...
  */
    function info_channelNameList()
    {
        ...
        if (!$cList = $this->info_channelList()) {
            return false;
        }
        $nList = array();
...
/**
  ...
  */
    function info_translateFlag($vFlag, $fType = 1, $oStr = false)
    {
        ...
        if ($fType != 1 && $fType != 2 && $fType != 3 && $fType != 4 && $fType != 5) {
            return false;
        }
        $decode = array();
        $decode[1] = array("CA", "O", "V", "AO", "AV");
//1 - See getPlayers() [11]
        $decode[2] = array("SA", "AR", "R", "", "ST");
//2 - See getPlayers() [12]
        $decode[3] = array("CC", "VR", "NW", "AW", "MM", "SM", "RC");
//3 - See getPlayers() [13]
        $decode[4] = array("U", "M", "P", "S", "D");
//4 - See getChannels() [7]
        $decode[5] = array("UU", "RU", "UC", "RC");
//5 - Internal for KickIdler
        $uDec = $decode[$fType];
        $cFlags = array();
        $cnt = 0;
        while ($vFlag > 0) {
  ...
  */
    function admin_clearChannel($cID, $kMode = 1, $param = null)
    {
        ...
        if ((!$param || !is_array($this->info_channelUser($param))) && $kMode == 1) {
            if (!$param = $this->info_defaultChannel()) {
                return false;
            } //Use the default channel if $param is invalid
        }
        $cmd = "";
        $cStat = true;
...
/**
  ...
  */
    function admin_dbUserList()
    {
        ...
        if (!$uRead = $this->_fastcall("DBUSERLIST")) {
            return false;
        }
        $uPattern = '/^([0-9]+)\t(.*)\t(.*)\t(.*)\t"(.*)"$/';
        $uRead = explode("\r\n", $uRead);
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->admin_dbUserList()) {
            return -1;
        }
        foreach ($uRead as $row) {
            if ($row[5] == $uName) {
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->_fastcall("DBSUSERLIST")) {
            return false;
        }
        $uPattern = '/^([0-9]+)\t(.*)\t(.*)\t"(.*)"$/';
        $uRead = explode("\r\n", $uRead);
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->sadmin_dbSUserList()) {
            return -1;
        }
        foreach ($uRead as $row) {
            if ($row[4] == $uName) {
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->_fastcall("FC $sQuery")) {
            return false;
        }
        $uPattern = '/^([0-9]+)\t([0-9]+)\t(.*)$/';
        $uRead = explode("\r\n", $uRead);
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->_fastcall("FP $sQuery")) {
            return false;
        }
...
/**
  ...
  */
    function sadmin_channelInfo($cID)
    {
        ...
        if (!$uRead = $this->_fastcall("CI $cID")) {
            return false;
        }
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->_fastcall("PI $cID")) {
            return false;
        }
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->_fastcall("DBCI $cID")) {
            return false;
        }
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->_fastcall("DBFP $sQuery")) {
            return false;
        }
        $uPattern = '/^([0-9]+)\t"(.*)"\t(.*)\t(.*)\t(.*)$/';
        $uRead = explode("\r\n", $uRead);
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->_fastcall("DBSERVERLIST")) {
            return false;
        }
        $uRead = explode("\r\n", $uRead);
        array_shift($uRead);
...
/**
  ...
  */
    ...
    {
        if (!$uRead = $this->_fastcall("BANLIST")) {
            return false;
        }
        $uRead = explode("\r\n", $uRead);
        array_shift($uRead);
...
/**
  ...
  */
    function wi_readServerSettings()
    {
        ...
        if (!preg_match_all($pattern, $sRead[1], $matches)) {
            return false;
        }
        $data = array();
  ...
  */
    function wi_writeServerSettings($sData = array())
    {
        ...
        foreach ($sData as $key => $val) {
            if (
                (!in_array($key, $sKeys) && !in_array($key, $ssKeys) && $this->isSAdmin) ||
