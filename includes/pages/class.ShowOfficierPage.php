...
class ShowOfficierPage
{
    ...
    private function addOfficier($USER, $Officier, $Mode)
    {
        ...
        if ($Officier == 600) {
            $offtime =  $USER['commander_time'];
        }
        if ($Officier == 601) {
            $offtime =  $USER['geologe_time'];
        }
        if ($Officier == 602) {
            $offtime =  $USER['admiral_time'];
        }
        if ($Officier == 603) {
            $offtime =  $USER['engineer_time'];
        }
        if ($Officier == 604) {
            $offtime =  $USER['technocratic_time'];
        }
        ...
        if ($Officier == 604 && $USER['technocratic'] == 0) {
            $db->query("UPDATE " . USERS . " SET `spy_tech`=spy_tech+2 WHERE id=" . $USER['id'] . "");
        }
